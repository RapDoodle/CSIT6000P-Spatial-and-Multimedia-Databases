    ...
    methods
        ...
        function push(obj, el) % 压入新元素到队尾
            if obj.size >= obj.capacity - 1
                sz = obj.size();
                new_buffer = cell(obj.capacity * 2, 1);
                if obj.rear >= obj.beg
                    new_buffer(1:sz) = obj.buffer(obj.beg:obj.rear-1);                    
                else
                    new_buffer(1:sz) = [obj.buffer(obj.beg:obj.capacity); obj.buffer(1:obj.rear-1)];
                end
